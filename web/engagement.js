...
window.Sub2MiniEngagement = (() => {
  ...
  function pageAdminTable(items) {
    return `<div class="table-wrap"><table><thead><tr><th>页面</th><th>类型</th><th>状态</th><th>排序</th><th></th></tr></thead><tbody>${items.map(item => `<tr><td><span class="cell-main">${escapeHtml(item.title)}</span><span class="cell-sub mono">${escapeHtml(item.slug)}</span></td><td>${item.kind === "legal" ? "法律文档" : "自定义"}</td><td>${item.enabled ? status("启用") : status("停用", "off")}</td><td>${item.sort_order}</td><td><div class="cell-actions"><button class="button quiet small" data-page-action="edit" data-id="${item.id}">编辑</button><button class="button quiet small" data-page-action="delete" data-id="${item.id}">删除</button></div></td></tr>`).join("")}</tbody></table></div>`;
  }
  ...
  async function saveContentPage(id) {
    ...
    values.enabled = form.querySelector("#page-enabled").checked;
